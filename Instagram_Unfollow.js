...
async function fillUsers(which) {
    ...
    while(true) {
        ...
        if (data != null) {
            if (data == "ERROR" || data == "RATELIMIT") {
                if (data == "RATELIMIT")
                    throw new Error("You've exceeded the rate limit. Try again in 24 hours.");
                else 
                    throw new Error("Error detected. Haulting script.");
            }
...
async function isCeleb(pk) {
    ...
    if (data != null) {
        if (data == "ERROR" || data == "RATELIMIT") {
            if (data == "RATELIMIT") {
                console.log("You've exceeded the rate limit. We'll try again in 5 minutes.")

                await new Promise(r => setTimeout(r, minutesToMilliseconds(5)));
            } else
                throw new Error("Error detected. Haulting script.");
        }
...
async function unfollow(unfollow_list) {
    try {
        for (var i = 0; i < unfollow_list.length; i++) {
            ...
            if (!celeb) {
                ...
                if (data == "ERROR" || data == "RATELIMIT") {
                    if (data == "RATELIMIT") {
                        console.log("You've exceeded the rate limit. We'll try again in 20 minutes.")
                        i -= 1;
                    ...
                    else throw new Error("Error detected. Haulting script.");
                }
...
async function main() {
    try {
        ...
        await unfollow(not_following_me_arr);
    } catch (error) {
